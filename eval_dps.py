...
def parse_filename_info(filename, is_sent=False):
    """
    Parses filenames based on bench_MMSE.py and mimo_dps_method_c.py outputs.
    
    Sent formats: 
      - "original_0.png" (generated by scripts)
      - "sentimg_0.png" (legacy)
    
    Recv formats:
      - "bench_snr-5_0.png"
      - "mmse_snr10_1.png"
      - "method_c_snr25_0.png"
    """
    name_no_ext = os.path.splitext(filename)[0]
    
    try:
        if is_sent:
            # Look for the last number in the string (the ID)
            # Matches "original_0", "image_15", etc.
            ...
            # Example: "method_c_snr-5_0" -> snr=-5, id=0
...
def plot_results(results, metric_name, t, r, mode):
    ...
    for i, (x_vals, y_vals, label) in enumerate(results):
        ...
        # Make Baseline (MMSE Linear) dashed and black for standard look
        if "Linear" in label or "NoSample" in label:
            c = 'black'
            l = '--'
        elif "Method C" in label:
            c = 'red'
            l = '-'
            m = 'o'
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate MIMO Diffusion Methods (MMSE vs DPS/Method C)")
    ...
    # 1. MMSE Benchmark Paths
    # bench_MMSE.pyの出力構造に合わせる: outputs/MIMO_Benchmark_MMSE/t={t}_r={r}/{mode}
    # nosampleは: outputs/MIMO_Benchmark_MMSE/t={t}_r={r}/nosample/{mode}
    
    base_experiment_mmse = f"outputs/MIMO_Benchmark_MMSE/t={args.t}_r={args.r}"
    
    # Standard Diffusion (Bench) path: .../t={t}_r={r}/{mode}
    path_mmse_bench = os.path.join(base_experiment_mmse, args.mode)
    
    # Linear (NoSample) path: .../t={t}_r={r}/nosample/{mode}
    # 修正: nosampleディレクトリの下にmode(estimated/perfect)が生成される構造に対応
    path_mmse_linear = os.path.join(base_experiment_mmse, "nosample", args.mode)

    # 2. Method C Paths
    methodc_dir_base = f"outputs/MIMO_MethodC_LS/t={args.t}_r={args.r}/{args.mode}"
    path_method_c = methodc_dir_base
    
    # Define Targets to evaluate: (Path, Label)
    targets = [
        (path_mmse_linear, "MMSE (Linear/NoSample)"),
        (path_mmse_bench,  "MMSE + Standard Diffusion"),
        (path_method_c,    "Proposed (Method C / DPS)"),
    ]
