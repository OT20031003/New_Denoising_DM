...
class OFDMMIMOSystem:
    ...
    def zf_equalize(self, y, h_est, noise_var=0.0):
        ...
        """
        # Permute to [B, T, F, Rx] and [B, T, F, Rx, Tx] for matmul
        y_perm = y.permute(0, 2, 3, 1).unsqueeze(-1) # [B, T, F, Rx, 1]
        h_perm = h_est.permute(0, 2, 3, 1, 4)         # [B, T, F, Rx, Tx]
...
def main():
    ...
    parser.add_argument("--snr", type=float, default=10.0, help="Test SNR (dB)")
    ...
    parser.add_argument("--ckpt_diff", type=str, default="diffusion_checkpoints/ckpt_step_135000.pth", help="Channel Estimator Checkpoint")
